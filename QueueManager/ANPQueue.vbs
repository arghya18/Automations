...
Wscript.echo queueCount & RS.RecordCount
...
If (insertErrNum = -2147217900) Then
	...
	SQL_query = "select Start_Time from Queue "&_
			"where Not(Start_Time) is Null and Command = "& utilityCMD & " order by queue_id asc;"
	
	Set oRS = CreateObject("ADODB.Recordset")	
	
	'oRS = MyConn.Execute(SQL_query)
	
	oRS.CursorLocation = 3
	oRS.Open SQL_query, MyConn, 3
	queueCount = oRS("Start_Time")

	Wscript.echo queueCount & oRS.RecordCount & "DateDiff" & DateDiff("n",oRS("Start_Time"),now())
