    ...
    if (!filename1.isEmpty()) {
        ...
        if (!filename2.isEmpty()) {
			...
			if (!filename3.isEmpty()) {
				int start = 0;
				int end = viewDiffComboBox->count();
				
				std::ofstream myfile;
				std::string textFileName;
				textFileName = filename3.toStdString();
				textFileName += ".txt";
				myfile.open (textFileName.c_str());
				if(end <= 1){
					myfile << "There are no differences found in the two PDF files.\n";		
				}
				else{
					myfile << "There are differences found in the two PDF files. Please refer to the attached PDF for details of differences.\n";
					QString header;
